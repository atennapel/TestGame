package com.atennapel.testgame;

public class Constants {
  public static final String SHEET_FILENAME = "sheet.atlas";
  public static final String SHEET_REGION = "sheet";
  public static final int SHEET_GRID = 16;
  public static final int SHEET_EMPTY = 16;

  public static final int WIDTH = 20;
  public static final int HEIGHT = 15;
  public static final int GRID = 32;

  public static final float ANIMATION_SPEED = 300f;
  public static final float BUMPING_RATIO = 0.2f;
  public static final float ATTACK_RATIO = 0.3f;
}
